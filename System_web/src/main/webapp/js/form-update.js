...
$("#form-submit").click(function (){
    ...
    $(".detail").each(function (index,ele){
        //标题空值
        var title=$(ele).children(":first-child").children(":nth-child(2)");
        if (title.attr("name")===null||title.attr("name")===undefined||title.attr("name")===""){
            var inputname="";
            inputname+=$(ele).attr("inputid")+"~";
            inputname+=(title.val()===null?title.prop("placeholder"):title.val())+"~title~";
        ...
        $(ele).children(":nth-child(2)").children().each(function (i,e){
            ...
            if (text.attr("name")===null||text.attr("name")===undefined||text.attr("name")===""){
                ...
                if ($(ele).attr("inputid")==="1"||$(ele).attr("inputid")==="2"){
                    textname+=($(e).children(":nth-child(2)").val()===""?$(e).children(":first-child").prop("placeholder"):$(e).children(":nth-child(2)").val())
                } else {
                    textname+=($(e).children(":first-child").val()===""?$(e).children(":first-child").prop("placeholder"):$(e).children(":first-child").val())
                }
